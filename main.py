...
import pb
from bot import P_TIMEZONE
from config import TIMEZONE_COMMON_NAME

bot = telebot.TeleBot("5109116614:AAFM5aHeCA7_54k8kUtJ_F0o5Oe3fe-lJ_w")

# Start chat
@bot.message_handler(commands=["start"])
def cmd_start(message):
 bot.send_message(message.chat.id, 'Hello! If you are interested in Currency exchange rates.\n' +
...
def help_command(message):
   ...
   keyboard.add(
       telebot.types.InlineKeyboardButton(
           'Message the developer', url='t.me/Currancyconverter8979Bot'
       )
   )
   bot.send_message(
       message.chat.id,
       ...
       '5) The bot supports inline. Type @<Currancyconverter8979Bot> in any chat and the first letters of a currency.',
...
def exchange_command(message):
    ...
    keyboard.row(
        ...
        telebot.types.InlineKeyboardButton('LKR', callback_data='get-LKR')
    )
